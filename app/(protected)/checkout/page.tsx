...
function CheckoutContent() {
    ...
    try {
      const paypalButtons = window.paypal.Buttons({
        createOrder: function(data: any, actions: any) {
          ...
          const currentTotal = parseFloat(searchParams.get('total') || '444000');
      ...
      if (paypalButtons.isEligible()) {
        paypalButtons.render(paypalRef.current).then(() => {
          ...
          console.log('PayPal button rendered successfully with amount:', totalAmount);
        }).catch((error: any) => {
  ...
  useEffect(() => {
    if (paypalLoaded && window.paypal && !paypalInitialized && paypalRef.current) {
      initializePayPal();
    }
                  ...
                  <div className="flex justify-between items-center py-2 border-b">
                    <div>
                      ...
                      <div className="text-sm text-muted-foreground">
                        {messageCount} messages/{billingType === 'yearly' ? 'year' : 'month'}
                      </div>
                    </div>
                    <span className="font-semibold">Rp {planPrice.toLocaleString('id-ID')}</span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b">
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="text-sm text-muted-foreground mb-4">
                    Your payment is secured by PayPal. You can pay with your PayPal account or credit card.
                  </div>
                  
                  {/* PayPal Button Container */}
                  <div ref={paypalRef} className="min-h-[50px] flex items-center justify-center">
                    {(!paypalLoaded || !paypalInitialized) && (
                      <div className="flex items-center gap-2 text-muted-foreground">
                        <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-primary"></div>
                        {!paypalLoaded ? 'Loading PayPal...' : 'Initializing payment...'}
                      </div>
                    )}
                  </div>
